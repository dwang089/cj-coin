...
class Transaction {
  ...
  update(sender, receiver, amount) {
    const output = this.output(output => output.address === sender.publicKey);
    ...
    sender.output.amount = output.amount - amount;
    this.output.push({amount: amount, address: receiver});
